...
def usage_message():
    ...
    print("pyatcoder binary -- numba aot でコンパイルしてバイナリで提出、AtCoder専用")
    print("pyatcoder compile -- numba aot でコンパイルしてバイナリで提出")
...
def main():
    if len(sys.argv) < 2 or sys.argv[1] not in ("combine", "aot", "binary", "compile", "sample", "submit", "pythran"):
        usage_message()
        sys.exit(-1)
    ...
    elif sys.argv[1] == "binary" or sys.argv[1] == "compile":
        binary_main(sys.argv[:2], args)
